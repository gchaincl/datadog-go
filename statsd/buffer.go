...
// writeHistogramAggregated serialized as many values as possible in the current buffer and return the position in values where it stopped.
func (b *statsdBuffer) writeHistogramAggregated(namespace string, globalTags []string, name string, values []float64, tags string, tagSize int) (int, error) {
	if b.elementCount >= b.maxElements {
	...
	b.buffer = append(b.buffer, '|')
	b.buffer = append(b.buffer, histogramSymbol...)
